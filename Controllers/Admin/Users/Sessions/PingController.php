...
class PingController extends \Rdb\Modules\RdbAdmin\Controllers\BaseController
{
    ...
    public function indexAction($user_id): string
    {
        ...
        if ($output['loggedIn'] !== true) {
            ...
            $output['formResultMessage'] = sprintf(__('Your session has been expired, please go to %1$slogin page%2$s.'), '<a class="rdba-login-page-link" href="' . $Url->getAppBasedPath(true) . '/admin/login">', '</a>');
